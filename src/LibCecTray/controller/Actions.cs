...
namespace LibCECTray.controller
{
  ...
  internal class Actions
  {
    ...
    private void ProcessEventHandler(object src, UpdateEvent updateEvent)
    {
      ...
      {
        case UpdateEventType.StatusReady:
          _controller.SetReady();
          break;
        case UpdateEventType.StatusText:
          _controller.SetStatusText(updateEvent.StringValue);
          break;
        case UpdateEventType.ClearWarning:
          _controller.CecWarnings.Remove(updateEvent.AlertValue);
          _controller.UpdateAlertStatus();
          break;
        case UpdateEventType.RegisterWarning:
          if (!_controller.CecWarnings.Contains(updateEvent.AlertValue))
            _controller.CecWarnings.AddLast(updateEvent.AlertValue);
          _controller.UpdateAlertStatus(); 
          break;
        ...
        case UpdateEventType.HasAVRDevice:
          // XXX also allow the AVR to be selected if it's not detected at this time
